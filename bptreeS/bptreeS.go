...
func (node *BPNode) findItem(key string) int {
	num := len(node.Items)
	for i:=0; i < num; i++ {
...
func (node *BPNode) setValue(key string, value interface{}) {
	item := StringKV{key, value}
	num := len(node.Items)
	if num < 1 {
		...
		node.Items = append([]StringKV{item}, node.Items...)
		return
	...
	for i:=0; i < num; i++ {
		if node.Items[i].Key > key {
			node.Items = append(node.Items, StringKV{})
...
func (node *BPNode) deleteItem(key string) bool {
	num := len(node.Items)
	for i:=0; i < num; i++ {
		if node.Items[i].Key > key {
			return false
...
func NewLeafNode(width int) *BPNode {
	var node = &BPNode{}
	node.Items = make([]StringKV, width+1)
...
func (t *BPTree) Get(key string) interface{} {
	t.mutex.Lock()
...
func (t *BPTree) getData(node *BPNode) map[string]interface{} {
...
func (t *BPTree) GetData() map[string]interface{} {
	t.mutex.Lock()
	defer t.mutex.Unlock()
...
func (t *BPTree) setValue(parent *BPNode, node *BPNode, key string, value interface{}) {
	for i:=0; i < len(node.Nodes); i++ {
...
func (t *BPTree) Set(key string, value interface{}) {
	t.mutex.Lock()
	defer t.mutex.Unlock()
...
func (t *BPTree) deleteItem(parent *BPNode, node *BPNode, key string) {
	for i:=0; i < len(node.Nodes); i++ {
		if key <= node.Nodes[i].MaxKey {
...
func (t *BPTree) Remove(key string) {
	t.mutex.Lock()
	defer t.mutex.Unlock()
